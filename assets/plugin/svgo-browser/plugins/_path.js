...
const rArcSeq =
  `${rNumberCommaWsp}?`.repeat(2) + rNumberCommaWsp + rFlagCommaWsp.repeat(2) + rCoordinatePair;
...
const { transformArc } = require('./_transforms.js');
const collections = require('./_collections.js');

const { referencesProps } = collections;
const defaultStrokeWidth = collections.attrsGroupsDefaults.presentation['stroke-width'];
  ...
  path
    ...
    .forEach((data) => {
      ...
      if (regPathInstructions.test(data)) {
        ...
        for (let pair = paramsLength[instruction]; data.length; ) {
          pathData.push({
            instruction,
...
exports.applyTransforms = function (elem, path, params) {
  ...
  if (
    ...
    elem.someAttr((attr) => ~referencesProps.indexOf(attr.name) && ~attr.value.indexOf('url('))
  )
    return path;
      ...
      if (
        ...
        if (elem.hasAttr('stroke-width')) {
          ...
          elem.addAttr({
            ...
            prefix: '',
            local: 'stroke-width',
            value: strokeWidth.replace(regNumericValues, (num) => removeLeadingZero(num * scale)),
          });
        }
      }
    }
...
exports.js2path = function (path, data, params) {
  ...
  path.attr('d').value = data.reduce((pathString, item) => {
    let strData = '';
    if (item.data) {
