...
const { applyTransforms } = require('./_path.js');
  ...
  path = path.filter((item, index, path) => {
    ...
    if (data) {
      ...
      if (
        ...
        ((instruction != 'h' && instruction != 'v') || prev.data[0] >= 0 == data[0] >= 0)
      ) {
        prev.data[0] += data[0];
...
function isArc(curve, circle) {
  ...
  return [0, 1 / 4, 1 / 2, 3 / 4, 1].every(
    (point) =>
      Math.abs(getDistance(getCubicBezierPoint(curve, point), circle.center) - circle.radius) <=
      tolerance
  );
}
